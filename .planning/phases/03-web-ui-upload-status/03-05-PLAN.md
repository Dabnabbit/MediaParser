...
<task type="auto">
  ...
  <files>app/static/img/placeholder.png</files>
  <action>
Create a simple placeholder image for files without thumbnails.

Since we can't create binary images programmatically, create a placeholder using SVG that can be served as a static file:
...
Update the results.js to use .svg extension:
- Change `/static/img/placeholder.png` to `/static/img/placeholder.svg`
  </action>
  <verify>
  ...
  <done>
    ...
    - results.js references correct placeholder path
  </done>
</task>
