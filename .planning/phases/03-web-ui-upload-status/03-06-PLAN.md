...
must_haves:
  truths:
    ...
    - "User can configure output directory"
---
...
curl -s -I http://localhost:5000/static/js/settings.js
```
  </action>
  <verify>
  ...
  <done>
    ...
    - All static files accessible (including settings.js)
    - API endpoints respond (even if with 404 for non-existent jobs)
    - Settings API responds with current configuration
  </done>
</task>
...
- Settings panel with output directory configuration
  </what-built>
  <how-to-verify>
**Setup:**
...
**Test 8: Settings (WEB-05)**
1. Click the gear icon / Settings toggle
2. Verify: Settings panel expands
3. Verify: Output directory field shows current path
4. Enter a new valid path (e.g., /tmp/mediaparser-output)
5. Click Save - verify "Saved!" feedback
6. Enter an invalid path (e.g., /root/noaccess or empty)
7. Click Save - verify error message displays
8. Click Reset - verify field returns to default
9. Refresh page - verify saved setting persists
...
- Settings not saving (API or persistence issue)
  </resume-signal>
</task>
...
- Settings panel allows output directory configuration (WEB-05)
- Invalid paths show validation errors
