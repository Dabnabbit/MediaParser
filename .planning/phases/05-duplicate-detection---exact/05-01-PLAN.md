...
2. For each file in a duplicate group, call get_quality_metrics() and add:
   - width: int | None
   - height: int | None
   - resolution_mp: float | None (megapixels)
   - format: str | None (from mime_type, e.g., "jpeg", "png")

3. After building each group, call recommend_best_duplicate(files) to get recommended_id
