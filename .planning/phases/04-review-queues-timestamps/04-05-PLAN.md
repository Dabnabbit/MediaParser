...
must_haves:
  truths:
    - "Native dialog element opens as modal when file clicked"
    - "File preview image/thumbnail displays in modal"
    - "Prev/Next navigation works with arrow keys and buttons"
    - "Escape key or X button closes the modal"
    - "File metadata displayed (filename, size, dimensions)"
  artifacts:
...
<objective>
Implement examination view modal using native HTML dialog element.

Purpose: Enable detailed file review with timestamp sources and navigation as specified in CONTEXT.md - modal approach for accessibility and focus management.
