...
must_haves:
  truths:
    ...
    - "API supports duplicate handling actions (not-duplicate, discard, select-best)"
  artifacts:
    - path: "app/models.py"
      provides: "Tag model and File model extensions"
      contains: "class Tag"
    - path: "app/routes/review.py"
      provides: "Review, tagging, and duplicate handling API endpoints"
      exports: ["review_bp"]
  key_links:
...
<objective>
Add database models and API endpoints for the review workflow including duplicate handling.

Purpose: Enable backend support for timestamp review decisions, file tagging, reviewed state tracking, and duplicate group management.
Output: Extended File model, new Tag model, review API endpoints, and duplicate handling endpoints.
</objective>
...
- `discarded: Mapped[bool]` - Default False, True if file excluded from output
- `duplicate_group_id: Mapped[Optional[str]]` - Nullable, set to file_hash when part of duplicate group, can be cleared to "un-duplicate"
...
```
  ...
  <done>File model has reviewed_at, final_timestamp, discarded, and duplicate_group_id fields. Tag model exists with name, usage_count, and many-to-many relation to File via file_tags table.</done>
</task>

<task type="auto">
  <name>Task 2: Create review and tagging API endpoints</name>
...
1. `GET /api/files/<file_id>` - Get single file with full details including timestamp_candidates
   - Returns: id, original_filename, detected_timestamp, final_timestamp, timestamp_source, confidence, reviewed_at, timestamp_candidates (parsed JSON), tags, file_size_bytes, mime_type, thumbnail_path, file_hash, discarded, duplicate_group_id
...
9. `POST /api/files/bulk/discard` - Bulk discard files (exclude from output)
   - Request body: { file_ids: [1,2,3] }
   - Sets discarded=True for each file
   - Returns success count

10. `POST /api/files/bulk/not-duplicate` - Remove files from their duplicate groups
    - Request body: { file_ids: [1,2,3] }
    - Clears duplicate_group_id for each file
    - Returns success count
...
```
  ...
  <done>Review blueprint registered, all 10 endpoints return appropriate responses (200/404/400 as expected), file review updates reviewed_at, tag operations work correctly, duplicate handling endpoints functional.</done>
</task>

<task type="auto">
  <name>Task 3: Extend jobs.py files endpoint with filters and sorts</name>
...
2. Extend response to include additional fields per file:
   ...
   - `is_duplicate` (boolean - true if file has duplicate_group_id set)
   - `file_hash` (for grouping duplicates on frontend)
   - `discarded` (boolean)

3. Support multiple confidence values in single request:
...
with app.app_context():
    client = app.test_client()
    # Test files endpoint with sort param
    resp = client.get('/api/jobs/999/files?sort=filename&order=desc')
    print(f'GET /api/jobs/999/files with sort: {resp.status_code} (404 expected)')
"
```
  </verify>
  <done>Jobs files endpoint supports reviewed filter, has_duplicates filter, discarded filter, sort options, and returns extended file data with is_duplicate and file_hash fields.</done>
</task>

<task type="auto">
  <name>Task 4: Add summary counts endpoint</name>
  <files>app/routes/jobs.py</files>
  <action>
Add summary counts endpoint `GET /api/jobs/<job_id>/summary`:
- Returns counts for filter chips: { high: N, medium: N, low: N, none: N, reviewed: N, duplicates: N, failed: N, total: N }
- Query the job's files and aggregate counts by:
  - confidence level (high, medium, low, none)
  - reviewed_at IS NOT NULL for reviewed count
  - files with duplicate_group_id IS NOT NULL for duplicates count
  - files with processing_error for failed count
  - total file count (excluding discarded files by default)
  </action>
  <verify>
```bash
cd /home/dab/Projects/MediaParser && .venv/bin/python -c "
...
```
  ...
  <done>Summary endpoint returns accurate counts for all filter categories including confidence levels, reviewed, duplicates, and failed.</done>
</task>
...
7. Duplicate handling: bulk discard and not-duplicate endpoints work
</verification>

<success_criteria>
- File model extended with reviewed_at, final_timestamp, discarded, duplicate_group_id fields
- Tag model exists with name, usage_count, many-to-many to File
- 10 review/tagging/duplicate API endpoints functional
